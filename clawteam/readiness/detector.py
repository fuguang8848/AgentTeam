...
from clawteam.readiness.config import DetectorConfig, PROVIDER_PROMPT_MARKERS, DEFAULT_PROMPT_MARKERS
        ...
        if not self.prompt_detected and len(last_lines) >= 2:
            ...
            for line in last_lines[-5:]:
                if self._contains_prompt_marker(line):
                    logger.debug("[%s] Fast Path: prompt marker detected in line: %s", 
                                 self.agent_id, line[:50])
    ...
    def _start_fast_path_fallback(self) -> None:
        ...
        def fallback_handler():
            if self.prompt_detected and not self.destroyed:
                logger.debug("[%s] Fast Path fallback: no subsequent writes, resolve ready", 
                             self.agent_id)
    ...
    def _start_timeout(self) -> None:
        ...
        def timeout_handler():
            if not self.destroyed and self._ready_future and not self._ready_future.done():
                logger.warning("[%s] Wait ready timeout after %d ms", 
                              self.agent_id, self.max_wait_ms)
        ...
        def quiescence_check():
            ...
            if screen == self.last_screen_snapshot:
                ...
                if stable_duration >= self.quiescence_threshold_ms:
                    logger.debug("[%s] Quiescence detection: screen stable for %d ms, resolve ready",
                                 self.agent_id, stable_duration)
                    self._resolve_ready(True)
            else:
