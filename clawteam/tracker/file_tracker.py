...
class FileChangeTrackerConfig:
    ...
    ignore_patterns: List[str] = field(default_factory=lambda: [
        "*.pyc", "*.pyo", "__pycache__/*", ".git/*", "*.swp", "*.swo",
        ".DS_Store", "node_modules/*", "*.log", ".coverage", "htmlcov/*"
    ])
    # 去抖间隔（毫秒）
    debounce_ms: int = 100
    # 是否自动开始监听
...
class FileChangeTracker:
    ...
    def register_session(self, session_id: str, agent_name: str, team_name: str = "",
                         task_id: str = "", working_directory: str = "") -> None:
        """注册活跃会话"""
        with self._lock:
    ...
    def _handle_watch_event(self, event: WatchEvent) -> None:
        ...
        try:
            ...
            if (event.change_type == ChangeType.modified and 
            ...
            file_change = FileChange.from_change_record(
                record,
                diff=diff_entry.diff if diff_entry else ""
            )
            ...
            if attribution.success:
                logger.info(f"File change attributed: {event.path} -> {attribution.agent_name} "
    ...
    def track_manual_change(self, file_path: str, change_type: str = "modified",
                           agent_name: str = "", session_id: str = "") -> FileChange:
    ...
    def get_changes(self, file_path: Optional[str] = None, 
