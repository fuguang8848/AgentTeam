"""Replace stats-grid content with performance stats"""
...
# Replace
content = content[:match.start()] + new_stats_grid + content[match.end():]
print(f"Replaced stats-grid ({len(old_stats_grid)} chars -> {len(new_stats_grid)} chars)")
