...
from agentteam.reader import OutputReaderManager, OutputEvent, OutputEventType, TokenUsage
...
def test_output_event_creation():
    ...
    event = OutputEvent(
        session_id="session-1", event_type=OutputEventType.MESSAGE, timestamp=1234567890.0, content="Hello, world!"
    )
...
def test_output_event_default_timestamp():
    ...
    event = OutputEvent(session_id="session-1", event_type=OutputEventType.MESSAGE, content="Test")
...
def test_token_usage():
    ...
    usage = TokenUsage(
        input_tokens=1000, output_tokens=500, total_tokens=1500, input_cost=0.01, output_cost=0.02, total_cost=0.03
    )
...
def test_emit_event_with_callbacks():
    ...
    event = OutputEvent(session_id="session-1", event_type=OutputEventType.MESSAGE, content="Test event")
