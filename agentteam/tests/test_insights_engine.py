...
def test_multiple_tools_recording(insights_engine):
    ...
    for tool in tools:
        for _ in range(3):  # 每个工具记录3次
            insights_engine.record_tool_usage(tool_name=tool, session_id=f"session_{tool}", metadata={"iteration": _})
...
def test_activity_patterns(insights_engine):
    ...
    for hour in [9, 10, 14, 15, 20]:  # 工作时间
        for _ in range(5):
            insights_engine.record_tool_usage(
                tool_name="test_tool", session_id=f"session_{hour}", metadata={"hour": hour}
            )
...
def test_metadata_handling(insights_engine):
    """测试元数据处理"""
    complex_metadata = {"nested": {"value": 42, "list": [1, 2, 3], "bool": True}, "string": "test", "number": 123.45}

    # 记录带复杂元数据的工具使用
    insights_engine.record_tool_usage(tool_name="metadata_test", session_id="meta_session", metadata=complex_metadata)
