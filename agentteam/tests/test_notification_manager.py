...
class TestNotificationManagerSpecificMethods:
    ...
    def test_on_task_completed(self):
        ...
        notification = manager.on_task_completed(
            session_id="test-session", session_name="Test Session", task_summary="Test task completed"
        )
    ...
    def test_on_error(self):
        """测试错误通知"""
        manager = NotificationManager()
        notification = manager.on_error(session_id="test-session", session_name="Test Session", error_msg="Test error")
        assert notification is None or notification.notification_type == NotificationType.ERROR

    def test_on_confirmation_needed(self):
        """测试确认请求通知"""
        manager = NotificationManager()
        notification = manager.on_confirmation_needed(session_id="test-session", session_name="Test Session")
        assert notification is None or notification.notification_type == NotificationType.CONFIRMATION

    def test_on_info(self):
        """测试信息通知"""
        manager = NotificationManager()
        notification = manager.on_info(session_id="test-session", title="Info", body="Test info")
        assert notification is None or notification.notification_type == NotificationType.INFO

    def test_on_warning(self):
        """测试警告通知"""
        manager = NotificationManager()
        notification = manager.on_warning(session_id="test-session", title="Warning", body="Test warning")
