...
def test_tool_usage_recording(engine):
    ...
    engine.record_tool_usage(
        tool_name="write_file", session_id="sess_001", team_id="team_a", task_id="task_001", metadata={"size": 1024}
    )
...
def test_generate_report_with_team_filter(engine):
    """测试按团队过滤的报告生成"""
    engine.record_tool_usage(tool_name="team_filter_test", session_id="sess_team", team_id="specific_team")
...
def test_metadata_none_handling(engine):
    """测试 None 元数据处理"""
    engine.record_tool_usage(tool_name="none_meta", session_id="sess_none", metadata=None)
...
def test_empty_session_id(engine):
    """测试空会话 ID 处理"""
    engine.record_tool_usage(tool_name="empty_sess", session_id="")
