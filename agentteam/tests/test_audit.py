...
def test_log_audit_event_append_only(temp_team_data):
    ...
    event_id1 = log_audit_event(
        team=team_name, event_type=AuditEventType.TASK_CREATED, actor="agent1", details={"subject": "First task"}
    )
    ...
    event_id2 = log_audit_event(
        team=team_name, event_type=AuditEventType.TASK_COMPLETED, actor="agent2", details={"result": "success"}
    )
...
def test_read_audit_log(temp_team_data):
    ...
    # Log some events
    event_id1 = log_audit_event(
        team=team_name, event_type=AuditEventType.TASK_CREATED, actor="agent1", details={"subject": "Task 1"}
    )
...
def test_get_audit_summary(temp_team_data):
    ...
    # Add some events
    log_audit_event(team=team_name, event_type=AuditEventType.TASK_CREATED, actor="agent1")

    log_audit_event(team=team_name, event_type=AuditEventType.TASK_COMPLETED, actor="agent2")
...
def test_timestamp_format(temp_team_data):
    ...
    team_name = "test-team"
    event_id = log_audit_event(team=team_name, event_type=AuditEventType.TASK_CREATED, actor="test")
