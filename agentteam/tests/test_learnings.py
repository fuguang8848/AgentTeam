...
from agentteam.learnings.auto_capture import LearningType, ExperienceEntry, AutoCaptureEngine, PatternDetector
...
class TestExperienceEntry:
    ...
    def test_experience_entry_defaults(self):
        """测试默认值"""
        entry = ExperienceEntry(entry_type=LearningType.BEST_PRACTICE, summary="最佳实践测试")
    ...
    def test_experience_entry_json_serialization(self):
        """测试 JSON 序列化"""
        entry = ExperienceEntry(entry_type=LearningType.LEARNING, summary="学习测试")
...
class TestPatternDetector:
    ...
    def test_calculate_pattern_confidence(self):
        ...
        # 测试错误集群模式
        error_pattern = {"type": "error_cluster", "error_types": ["timeout"]}

        confidence = detector.calculate_pattern_confidence(error_pattern, [])
        assert 0.0 <= confidence <= 1.0

        # 测试工具模式
        tool_pattern = {"type": "tool_pattern", "tools": ["bash"]}
...
class TestAutoCaptureEngine:
    ...
    def test_evaluate_task_result_with_feedback(self, engine):
        """测试带用户反馈的任务评估"""
        task_result = {"status": "success", "task_id": "test-task-123", "session_id": "test-session-456"}
    ...
    def test_detect_area(self, engine):
        """测试检测任务领域"""
        # 测试前端任务
        frontend_task = {"tools_used": ["html", "css", "javascript"], "description": "创建React组件"}
        area = engine._detect_area(frontend_task)
        assert area == "frontend"

        # 测试后端任务
        backend_task = {"tools_used": ["python", "api", "database"], "description": "实现REST API"}
        area = engine._detect_area(backend_task)
        assert area == "backend"

        # 测试测试任务
        test_task = {"tools_used": ["pytest", "test"], "description": "编写测试用例"}
        area = engine._detect_area(test_task)
        assert area == "tests"

        # 测试通用任务
        general_task = {"tools_used": [], "description": "常规任务"}
    ...
    def test_record_experience_new(self, engine, temp_learnings_dir):
        """测试记录新经验"""
        entry = ExperienceEntry(
            entry_type=LearningType.ERROR, summary="新错误", details="这是一个新错误", area="backend"
        )
    ...
    def test_generate_learning_summary(self, engine):
        ...
        recent_entry = ExperienceEntry(
            entry_type=LearningType.ERROR, summary="近期错误", details="今天发生的错误", priority="high"
        )
    ...
    def test_list_experiences(self, engine):
        ...
        entries = [
            ExperienceEntry(
                entry_type=LearningType.ERROR, summary="错误1", details="未解决的错误", area="backend", resolved=False
            ),
            ExperienceEntry(
                entry_type=LearningType.BEST_PRACTICE,
                summary="最佳实践1",
            ...
            ExperienceEntry(
                entry_type=LearningType.ERROR, summary="错误2", details="另一个错误", area="infra", resolved=False
            ),
        ]
    ...
    def test_mark_as_resolved(self, engine):
        """测试标记为已解决"""
        entry = ExperienceEntry(entry_type=LearningType.ERROR, summary="待解决错误", details="需要解决的错误")
