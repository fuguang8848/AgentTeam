...
class SSEHandlersMixin:
    ...
    def _sse_events(self):
        from agentteam.board.sse.broadcast import _event_queue, _event_subscribers, _event_broadcaster_lock, _register_event_subscriber
    ...
    def _sse_activity(self):
        from agentteam.board.sse.agent_activity import _agent_activity_queue, _agent_activity_subscribers, _agent_activity_lock
        params = parse_qs(urlparse(self.path).query)
        team, agent = params.get("team", [None])[0], params.get("agent", [None])[0]
    ...
    def _stream_sse(self, queue, subscribers, lock, team, agent):
        ...
        try:
            ...
            while True:
                ...
                if acquired: sub_lock.release()
                with lock:
                    while len(queue) > last_idx:
                        d = queue[last_idx]
                        if (team is None or d.get("team_name") == team) and (agent is None or d.get("agent_name") == agent):
                            self.wfile.write(f"data: {json.dumps({'type': 'event', 'data': d}, ensure_ascii=False)}\n\n".encode())
                            self.wfile.flush()
                        last_idx += 1
            ...
            with lock:
                if sub_lock in subscribers: subscribers.remove(sub_lock)
        ...
        try:
            with _subscriber_lock:
                for e in list(_chat_event_queue)[-50:]:
                    self.wfile.write(f"data: {json.dumps({'type': 'chat_event', 'data': e}, ensure_ascii=False)}\n\n".encode())
                    self.wfile.flush()
            while True:
                acquired = sub_lock.acquire(timeout=10)
                if acquired: sub_lock.release()
                with _subscriber_lock:
                    while len(_chat_event_queue) > last_idx:
                        self.wfile.write(f"data: {json.dumps({'type': 'chat_event', 'data': _chat_event_queue[last_idx]}, ensure_ascii=False)}\n\n".encode())
                        self.wfile.flush()
                        last_idx += 1
                self.wfile.write(f"data: {json.dumps({'type': 'heartbeat'}, ensure_ascii=False)}\n\n".encode())
                self.wfile.flush()
            ...
            with _subscriber_lock:
                if sub_lock in _chat_subscribers: _chat_subscribers.remove(sub_lock)
