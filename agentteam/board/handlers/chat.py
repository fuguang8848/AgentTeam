...
class ChatHandlersMixin:
    ...
    def _chat_send(self):
        ...
        if not msg: return self.send_error(400, "Message required")
        resp = handle_chat_command(msg, user)
        if resp.get("content") != "CLEAR_CHAT_HISTORY":
            _chat_event_queue.append({"type": "message", "role": resp.get("role", "assistant"),
                "content": resp.get("content", ""), "timestamp": resp.get("timestamp", _now_iso()), "user": user})
            self._notify_chat()
    ...
    def _chat_save(self):
        ...
        payload = self._parse_json_body()
        if payload is None: return
        _chat_event_queue.append({"type": "message", "role": payload.get("role", "user"),
            "content": payload.get("content", ""), "timestamp": payload.get("timestamp", _now_iso()), "user": payload.get("user", "User")})
        self._notify_chat()
    ...
    def _import_tmpl(self):
        ...
        try:
            ...
            name = tmpl.get("name", "")
            if not name: return self.send_error(400, "Template name required")
